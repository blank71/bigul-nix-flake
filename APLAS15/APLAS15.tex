...
\spnewtheorem*{theorem*}{Theorem}{\rm\bf}{\sl}
...
Putback-based bidirectional programming allows us to write only one putback 
transformation, from which the unique corresponding forward transformation is derived for free.
Although a putback transformation has full control of bidirectional behaviour, 
its logic is more sophisticated than that of a forward transformation and does not always give rise to well-behaved bidirectional programs. This calls for design of 
...
to serve as the core for higher-level putback-based languages,
...
which is intended to serve as a clean and safe foundation for higher-level putback-based languages (e.g., \BiFluX),
...
After explaining how to formalise and prove well-behavedness of BXs in \Agda\ in \autoref{sec:BX}, we present |BiGUL| and its proof-carrying bidirectional semantics in \autoref{sec:BiGUL}, and discuss several issues (in particular defend the putback-based approach) in \autoref{sec:discussion}.
For the benefit of the readers not familiar with \Agda, we provide an appendix explaining \Agda\ syntax.
Also, all important \Agda\ identifiers are hyperlinked to their definitions, which should be helpful to the readers who read this paper electronically.
...
Our partial transformations are actually total functions which wrap their results in the standard |^^^Maybe| datatype whose two constructors are |^^^just : A -> Maybe A| and |^^^nothing  : Maybe A| --- the result of a successful computation is wrapped in the |just| constructor, while failed computation is represented by |nothing|.
It is thus always possible to know whether a computation succeeds or not in a finite amount of time.
This notion of ``decidable partiality'' should be distinguished from the usual partiality as formalised in terms of complete partial orders (CPOs; see, e.g., Gibbons~\cite{Gibbons-calculating-functional-programs}):
when defining the semantics of |BiGUL|, we need to write programs that produce some result or fail respectively when a sub-program fails or produces some result, but this is not possible in the CPO setting.
...
To achieve this, we need to reify the monadic binding structure of |Maybe|-programs by deeply embedding the monadic combinators as the constructors of a datatype, so later we can define types of proofs of successful computation by induction on the binding structure.
...
\end{code}
For convenience, there are also other operators for error-handling and assertion, which we omit from the presentation.
...
Usually these languages are designed to suggest that what they describe are |get| programs, but it has recently been argued that the opposite \emph{putback-based} approach should be adopted instead due to the following theorem:
...
The types of the |BiGUL| constructors are indexed by the source and view types expected by the operations represented by the constructors, and ultimately we can write a well-typed interpreter
\begin{code}
...
Due to space limitation, we will not present the full definitions of |interp| nor the auxiliary types used in \autoref{fig:BiGUL}, which are actually straightforward translation to and deep embedding of |BiGUL|'s lens semantics.
...
The idea itself is straightforward; what follows, despite its slight complexity (especially if the reader is not familiar with dependently typed programming), is merely our datatype-generic~\cite{Gibbons-DGP,Altenkirch-GP-within-DTP} and strongly typed implementation of the idea in \Agda.
...
We can now define a family of types |Pattern : U -> Set| such that, for any |D : U|, the type |Pattern D| contains exactly those patterns sensible for |⟦ D ⟧| --- for example, on all types we can use the variable pattern |var : {D : U} -> Pattern D| which matchs anything, while on sum types we can use the patterns matching |inj₁| and |inj₂| constructors but not the pair pattern |prod : {D E : U} -> Pattern D ->| |Pattern E -> Pattern (D ⊗ E)|, which only makes sense for product types.
...
\end{code}
where |^^^Path pat T| is the type of all paths that lead from the root of a |PatResult pat| to a component of type |⟦ T ⟧| at a variable position.
...
Thus, in the definition of |CaseVBranch| below, we use a partial isomorphism on the view type --- e.g., an instance of |pat-iso| for doing pattern matching --- which converts it to a possibly less informative type, instead of just a decidable predicate as in |CaseSBranch|:
\begin{code}
...
One might raise doubts about how |BiGUL| can be justified as a putback-based language, as the fundamental definition of |Lens| is exactly the classic symmetric definition, and the design of |put| is still entangled with |get|.
We argue, though, that it is the programmability solely in terms of |put| that characterises putback-based bidirectional programming.
...
By shifting to the putback-based perspective, it becomes more natural to include more sophisticated updating operations that can be unintuitive to appear in a |get|-based design.
...
One might also be sceptical about the use of dynamic checks to guarantee the well-behavedness of |BiGUL|, as the programmer can write programs that fail inadvertently and only realise that at runtime.
...
We did not take this step because we plan to jump further by switching to a dependently typed setting where constraints on programmer-supplied actions can be directly specified in their types, and eliminating all dynamic checks from the lens semantics.
...
The dynamic checks are sometimes tricky to get right, and proofs about them can require big case analyses that are not so easy to manage.
...
Thus the fact that the whole |BiGUL| language is formally verified is a firm step towards reliable putback-based bidirectional programming: as |BiGUL| is intended to serve as the (new) core of higher-level putback-based languages like \BiFluX~\cite{Pacheco-BiFluX} and \BiYacc~\cite{Zhu-BiYacc},\footnote{Due to space limitation, we omit two minor features of |BiGUL| from this paper, which are necessary operations for \BiFluX, though. (But they are not too interesting anyway.)} the well-behavedness of the latter languages will be established beyond doubt.
