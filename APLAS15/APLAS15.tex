...
In this paper, we take one further step along the direction of \Putlenses\ and \BiFluX\ by focusing on a small yet sufficiently powerful putback-based core language called |BiGUL| (for \emph{Bidirectional Generic Update Language}),
...
For the rest of the paper, after explaining how to formalise and prove well-behavedness of BXs in \Agda\ in \autoref{sec:BX}, we present |BiGUL| and its proof-carrying bidirectional semantics in \autoref{sec:BiGUL}, and conclude with several remarks in \autoref{sec:concluding-remarks}.
...
fail-lens = iso-lens (record {to = \ _ -> fail SEP from = \ _ -> fail SEP ABBRV })
\end{code}
...
The types of such containers
\begin{code}
^^^VarPositions : (HIDE'({lv : Level})) {D : U} -> Pattern D -> (U -> Set (HIDE'(lv))) -> Set (HIDE'(lv))
\end{code}
can be defined by induction on |Pattern|.
...
Below we conclude this paper by contrasting putback-based BX with |get|-based BX, discussing issues about dynamic checks, comparing |BiGUL| with its closest relative \Putlenses, and talking briefly about porting |BiGUL| to \Haskell.
...
A |get|-based language can hardly be designed to purely describe |get| programs, since its expressive power would be severely limited if the behaviour of |put| cannot be customised; the way out is thus to parametrise |get| programs with |put| strategies, but then the programmer is forced to think in terms of both |get| and |put|, greatly complicating the logic.
In contrast, for a putback-based language, it is entirely natural to extend the language with more sophisticated updating operations, as we are simply enriching the putback logic.
We believe that the revised source case analysis with adaptation~(\autoref{sec:source-case}) is a particularly nice example of such operations.
...
Thus the fact that the whole |BiGUL| language is formally verified is a firm step towards reliable putback-based bidirectional programming: as |BiGUL| is intended to serve as the (new) core of higher-level putback-based languages like \BiFluX~\cite{Pacheco-BiFluX} and \BiYacc~\cite{Zhu-BiYacc},\footnote{Due to space limitation, we omit a minor feature of |BiGUL| (for stating dependencies between parts of the view) from this paper, which is necessary for \BiFluX, though.} the well-behavedness of the latter languages will be established beyond doubt.
