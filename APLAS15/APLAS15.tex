...
(Full source code is available at \url{http://www.prg.nii.ac.jp/project/bigul}.)
...
This paper is typeset with a modified version of lhs2\TeX\ such that all important \Agda\ identifiers are hyperlinked to their definitions, which should be helpful to the readers who read this paper electronically.
...
This notion of ``decidable partiality'' should be distinguished from the usual partiality as formalised in terms of complete partial orders:
...
Then for any |S|,~|V : Set| and two lenses |l|,~|r : Lens S V| whose |put| components are extensionally equal, i.e.,
...
data ^^^BiGUL : U → U → Set₁ where
  ...
  ^^^rearr    :  (HIDE({S V V' : U} →))  (pat : Pattern V) (pat' : Pattern V') →
                                         Paths pat pat' → BiGUL S V' → BiGUL S V
\end{code}
\end{center}
\afterfigure
...
Due to space limitation, we will not present the full definitions of |interp| nor the auxiliary types used in \autoref{fig:BiGUL}, which are actually straightforwardly derived from |BiGUL|'s lens semantics.
...
\begin{code}
^^^replace-lens : (HIDE({S : Set} ->)) Lens S S
...
\begin{code}
^^^fail-lens : (HIDE({S V : Set} ->)) Lens S V
...
align-lens : (HIDE({S V : Set} ->))
  (source-condition : S → Par Bool) (match : S → V → Par Bool)
...
We can now define a family of types |^^^Pattern : U -> Set| such that, for any |D : U|, the type |Pattern D| contains exactly those patterns sensible for |⟦ D ⟧| --- for example, on all types we can use the variable pattern |^^^var : (HIDE({D : U} ->)) Pattern D| which matchs anything, while on sum types we can use the patterns matching |inj₁| and |inj₂| constructors but not the pair pattern |^^^prod : (HIDE({D E : U} ->)) Pattern D ->| |Pattern E -> Pattern (D ⊗ E)|, which only makes sense for product types.
...
\begin{code}
^^^Lenses : (HIDE({D : U} ->)) Pattern D -> Set1
...
\begin{code}
^^^update-lens : (HIDE({D : U} ->))
  (pat : Pattern D) (ls : Lenses pat) -> Lens (PatResult pat) (Views pat ls)
\end{code}
...
\begin{code}
view-rearrangement-iso : (HIDE({D E : U} ->))
  (pat : Pattern D) (pat' : Pattern E) →
...
%
%A datatype can be \emph{parametrised}, as in the case of |CaseSBranchType|, which has two (type) parameters |S : Set| and |V : Set|, specified between the name of the datatype and the colon on the first line.
%(|Set| is the type of all (small) types.)
%Parameters can be used throughout a datatype definition, and should appear uniformly in all occurrences of the datatype being defined.
%(For example, in the types of the constructors of |CaseSBranchType| we should always use |CaseSBranchType S V|.)
%
%A datatype can also be \emph{indexed}: For example, the datatype |Par| is indexed by |Set| (which appears to the right of the colon on the first line), meaning that |Par| is in fact a |Set|-indexed family of simultaneously defined types.
%In a datatype definition, indices can appear non-uniformly in occurrences of the datatype being defined.
%For example, in the type of~|_>>=_| we use both |Par A| and |Par B| where |A|~and~|B| are different indices.
%
%\feature{Dependent functions}%
%In dependently typed languages like \Agda, the output type of a function can depend on the value of the input.
%In \Agda\ such a \emph{dependent function type} is written like |(x : A) -> B x|, where |B : A -> Set| is a type-computing function.
%Logically, dependent function types are read as universally quantified propositions --- the above dependent function type is read as ``for all |x : A| the property~|B| holds for~|x|''.
%(Ordinary function types, on the other hand, denote implications.)
%
%\feature{Implicit arguments}%
%When \Agda\ can infer what the value of an argument of a function should be based on how the function is used most of the time, we can make the argument \emph{implicit} in the function's type by enclosing it in curly brackets, like |{x : A} -> B x|.
%Then the function can be invoked without giving the implicit argument.
%We de-emphasise implicit arguments typographically when they cause visual annoyance.
%
%\feature{Records}%
%An \Agda\ record is defined by listing the names and types of all its components, and a component type can depend on values of previous components.
%To access a component of an inhabitant of a record type, we should use the corresponding component extraction function.
%For example,
%\begin{code}
%Lens.get : {S V : Set} -> Lens S V -> S -> Par V
%\end{code}
%is for extracting the |get| component of a |Lens|.
%
