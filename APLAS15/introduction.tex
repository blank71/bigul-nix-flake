...
Many bidirectional programming languages~\cite{Lenses,Bohannon:06,MHNHT07,Hidaka:10,PachecoCunha:10,Barbosa:2010,Hofmann:2011,FoPP08,HoPW12} have been designed
...
Most existing approaches to BX are $get$-based, in the sense that it asks users to write BX programs in terms of $get$ describing how to construct an abstract view form the source, and derives a specific backward $put$ for free.
As there are usually more than one {\em incomparable} $put$ for a $get$ and the $get$-based approaches provide only one $put$, the backward behavior becomes ambiguous and unpredictable.
%
To remedy this situation, a new approach called putback-based bidirectional programming~\cite{FiHP15} was proposed recently to support writing BX in terms of $put$ instead of $get$,
...
The combinator library $putlenses$~\cite{PaHF14} is such a language to support this approach. It provides a set of combinators for writing complex BX programs in the putback-based way.
In addition, \textsc{BiFlux}~\cite{PaZH14} is a bidirectional function update language for XML which is a carefully designed putback-based BX language. 
...
the source and the view and specified how to reflect the view to various changes on the source.
...
The language \BiGUL\ is an abstraction of the putback-based language 
...
But different from \textsc{BiFluX},
it is unique in that it
refines the case analysis operations to include the {\em adaptation} mechanism 
that can effectively deal with failure during the analysis and make 
