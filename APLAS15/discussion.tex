...
While none of them managed to fully prove the well-behavedness in a formal way.
In this paper, we design the minimalist putback-based core language
that is fully certified in \Agda. 
We defined a ``proof carried lens",
thus the semantics of each one is proved to satisfy the PutGet and GetPut properties by the real \Agda\ code
which is really runnable.
%

Our design of \BiGUL\ is based on the real putback-based language \textsc{BiFluX}.
...
we omit to show other two features in \BiGUL\ which is proved and implemented
that is view dependency and iteration on source. 
These are two necessary features when implementing the \textsc{BiFluX}.
...
and only one of them need to be used for updating the source.
Another one can be recovered from the view dependency when deriving $get$.
%
...
Iteration on source means update a source list with the same view element, while we need to guarantee the implicit constraint that
the elements in this list must be equal.
