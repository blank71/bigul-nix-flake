...
transformations: the {\em forward} transformation \ensuremath{\Varid{get}\;\Varid{s}} is used to
produce a target view \ensuremath{\Varid{v}} from a source \ensuremath{\Varid{s}}, while the {\em putback}
transformation \ensuremath{\Varid{put}\;\Varid{s}\;\Varid{v}} is used to reflect modifications on the view
\ensuremath{\Varid{v}} to the source \ensuremath{\Varid{s}}.  These two transformations should be {\em
...
the change of \ensuremath{\Varid{get}}. 
...
paired with \ensuremath{\Varid{put}} to form a bidirectional transformation if it exists.
...
Before explaining how to write \ensuremath{\Varid{put}}, let us briefly review
...
The first interesting fact is that, for a valid \ensuremath{\Varid{put}}, there exists at most one \ensuremath{\Varid{get}}
...
\end{align*}
Note that the view determination implies that \ensuremath{\Varid{put}\;\Varid{s}} is injective (with \ensuremath{\Varid{s}\mathrel{=}s^\prime}).
...
\begin{theorem}
\label{th:put2}
A \ensuremath{\Varid{put}} function is valid if and only if it satisfies the \ref{PutDet} and \ref{PutStable} properties. 
\end{theorem}
