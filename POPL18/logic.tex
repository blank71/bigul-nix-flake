...
Well-behavedness implies that a putback transformation uniquely determines the corresponding forward transformation --- that is, for any |l, r : S <~ V|, if |put l = put r| then |get l = get r|~\citep[Lemma~2.2.5]{Foster-thesis}.
...
(As \autoref{thm:putback-soundness-and-completeness} shows that the proof rules precisely capture \BiGUL's |put| semantics, we take \autoref{fig:putback-proof-rules} to be the formal definition of \BiGUL's (axiomatic) semantics in this paper, and will not give the definition of |put| formally.)
...
Most of \BiGUL's expressive power stems from its |case| construct, which has gone beyond \citet{Foster-lenses}'s ``general conditional'' in terms of expressiveness, allowing, in particular, key-based list alignment to be implemented only using simple primitives for the first time.
...
(The only comparable but still much less powerful reasoning framework in the existing literature is \citet{Foster-lenses}'s totality lemmas, which can establish properties equivalent to triples of the form |(ASSERTRANGE(_ _)) b (ASSERTRANGE P)| and |(ASSERT(s v || P s && Q v)) b (ASSERT(_ _ _))| where |P : (POWER S)| and |Q : (POWER V)|.)
