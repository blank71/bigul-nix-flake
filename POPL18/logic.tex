...
\end{code}
where the range triple is particularly interesting as it has a nontrivial output range.
...
We plan to adapt the Hoare-style logic for easier mechanised verification, in particular automated theorem proving (perhaps in the style of \name{LiquidHaskell}~\citep{Vazou-LiquidHaskell}), so as to verify larger-scale bidirectional programs with reasonable effort.
