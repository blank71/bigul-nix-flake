...
%\documentclass[acmlarge,fleqn,anonymous,review]{acmart}\settopmatter{printfolios=true}
...
%format (SUB(x)) = "\kern-1.25\smallskipamount_{" x "}"
%format ss'' = "\widetilde{ss}"
%format s'' = "\tilde{s}"
%format Pat = "\mathsf{Pat}"
%format Seq = "\mathsf{Seq}"
...
The putback-based approach to bidirectional programming was proposed to address this issue once and for all, and this paper takes the approach one step further by proposing a Hoare-style logic for \citet{Ko-BiGUL}'s putback-based language \BiGUL.
...
One such framework is \citet{Foster-lenses}'s \emph{asymmetric lenses}, which are highly influential such that the term \emph{bidirectional programming} has become largely synonymous with lens-based approaches (including lens combinators and bidirectionalisation; see, e.g., \citep{Foster-bidirectional-programming-approaches}).
...
Following some previous work which took exactly this \emph{putback-based} programming approach~\citep{Pacheco-putlenses, Pacheco-BiFluX, Hu-putback-validity}, \citet{Ko-BiGUL} proposed a language \BiGUL\ (short for the Bidirectional Generic Update Language).
...
A \BiGUL\ program~|b| (whose possible forms are summarised in \autoref{fig:BiGUL}) operating on source type~|S| and view type~|V| is assigned the type |S <~ V|, and has two semantics (interpretations):
\begin{code}
put  b : S -> V ->  Maybe S
get  b : S ->       Maybe V
\end{code}%
...
Well-behavedness implies that, for any |l, r : S <~ V|, if |put l = put r| then |get l = get r|.
(This is originally \citet{Foster-thesis}'s Lemma~2.2.5.)
That is, once a putback transformation is specified, the corresponding forward transformation is uniquely determined.
...
\begin{notation}
Relations on types |A(SUB 1)|\,, |A(SUB 2)|\,, $\ldots,$ |A(SUB n)| are considered inhabitants of the type |POWER(A(SUB 1) TIMES A(SUB 2) TIMES {-"\cdots\;"-} TIMES A(SUB n))|.
When a relation~|R| of this type relates elements |a(SUB 1) : A(SUB 1)|\,, |a(SUB 2) : A(SUB 2)|\,, $\ldots,$ |a(SUB n) : A(SUB n)|\,, we write |R a(SUB 1) a(SUB 2) ... a(SUB n)|\,. %(in a curried form, to save parentheses and commas).
\end{notation}
...
The intended meaning of a putback triple |(ASSERT R) b (ASSERT R')| is more or less standard: if the original source and the view satisfies the precondition~|R|, then |put b| will successfully produce an updated source satisfying the postcondition~|R'|, which can relate the updated source to the original source and the view.
...
We have proved in \Agda\ that putback triples are sound and complete with respect to \BiGUL's |put| semantics.
...
\begin{definition}
A \emph{comprehension relation} of type |(POWER(A(SUB 1) TIMES A(SUB 2) TIMES {-"\cdots\,"-} TIMES A(SUB n)))| has the form
\[ |COM(pat(SUB 1) pat(SUB 2) ... pat(SUB n) || prop)| \]
where each |pat(SUB i)| is a pattern for elements of type~|A(SUB i)| and |prop| is a proposition that can refer to the variables in the patterns.
The patterns we use in the paper include variables, constructors, and the wildcard pattern~(`|_|').
The relation holds for |a(SUB 1) : A(SUB 1)|, |a(SUB 2) : A(SUB 2)|, $\ldots,$ |a(SUB n) : A(SUB n)| exactly when each |a(SUB i)| matches |pat(SUB i)| and |prop| holds after substituting the matched components for the corresponding pattern variables.
\end{definition}
...
\caption{\BiGUL\ constructs and their typing (where |Pat| and |Seq| are hypothetical type constructors for patterns and sequences respectively)}
\label{fig:BiGUL}
...
& \quad |NormalDomain = BIGCUP ~ [ (NORM M) || (normal M ...) `elem` bs ]| \\ \cline{1-1}
\multicolumn{1}{c}{|(ASSERT(R CAP Domain)) ~ case bs ~ (ASSERT R')|}
& \quad |{-"\awa{\mathcal N}{\mathcal D}\;"-} = BIGCUP ~ [ M || (normal OR adaptive M ...) `elem` bs ]|
\end{array}$
...
Now we introduce \BiGUL's constructs (\autoref{fig:BiGUL}) and their putback proof rules (\autoref{fig:putback-proof-rules}).
\autoref{thm:putback-soundness-and-completeness} shows that the proof rules precisely capture \BiGUL's |put| semantics, and therefore we take \autoref{fig:putback-proof-rules} to be the formal definition of \BiGUL's (axiomatic) semantics in this paper and will not give the |put| semantics formally, although we will still give the operational intuition of each construct when explaining the corresponding proof rule.
...
%For each \BiGUL\ construct, we will introduce its type, which is essential for inferring the types of entities in assertions, and explain the corresponding proof rule with the help of an operational intuition about the construct.
...
Also, all assertions are strongly typed according to \autoref{def:putback-triples}; for example, the source and view types of |replace| are required to be the same; consequently, |s'|~and~|v| in the corresponding rule's postcondition are of the same type, and it is sensible to talk about equality between them.
...
In general, a case analysis in \BiGUL\ has the form |case bs : S <~ V| where |bs|~is an arbitrary number of |normal| or |adaptive| branches.
...
Requirement~(i) is for guaranteeing well-behavedness, while requirement~(ii) makes exit conditions analogous to ``assertions'' in reversible conditionals~\citep{Yokoyama-Janus}.
...
Regarding our revised version of \BiGUL, most of its expressive power stems from the |case| construct, which has gone beyond \citet{Foster-lenses}'s ``general conditional'' in terms of expressiveness, allowing, in particular, key-based list alignment to be implemented directly.
...
In contrast, \BiGUL\ is designed with the ultimate aim of expressing all lenses using just a fixed set of simple primitives, like what we can do in general-purpose programming languages --- we can program alignment in \BiGUL\ without having to bake special-purpose concepts like \citeauthor{Barbosa-matching-lenses}'s ``chunks'', ``rigid complements'', and ``resources'' into the language.
...
Their programs can be (and are usually) enriched with putback information to gain more control, but that makes constructing and understanding the programs more awkward (see the next paragraph).
In contrast, \BiGUL's putback-based design lets the programmer construct programs purely in the |put| direction.
The Hoare-style logic makes the distinction between the two approaches absolutely clear for the first time --- it is possible to precisely reason about bidirectional behaviour purely in the putback direction, whereas it is unthinkable that the same effect can be achieved from the opposite direction.

\paragraph{Wasn't there already previous work on controlling the putback behaviour?}

To name a few, the ``fixup functions'' in \citet{Foster-lenses}'s ``general conditionals'', the parameters of \citet{Bohannon-relational-lenses-tech-report}'s $\mathtt{join\_template}$, and the alignment keywords like ``\texttt{key}'' and ``\texttt{best}'' in Boomerang~\citep{Bohannon-Boomerang,Barbosa-matching-lenses} are all constructs which appear in programs designed to look like forward transformations but are meaningful only in the putback direction.
Constructs with similar purposes can also be found in bidirectionalisation approaches, such as shape bidirectionaliser plug-ins~\citep{Voigtlander-shape-plug-ins}.
The fundamental problem with these languages is that their programs contain a mixture of forward and backward information, and to understand such programs, the only way is to reason in both directions and in terms of the complex underlying semantics.
In other words, it is hard to come up with easy-to-use reasoning principles for these languages, and since reasoning principles reflect and even guide how we program, this indicates that these languages fail to deliver an effective abstraction.
\BiGUL\ is unique since it offers a successful abstraction in which programs are unidirectional, as clearly reflected in the Hoare-style logic.
...
More broadly, we believe that programming languages should be shipped with reasoning principles --- even domain-specific languages deserve domain-specific reasoning principles, to justify that the languages offer an adequate abstraction, and to help the programmer to work effectively and reliably with that abstraction.
In the case of \BiGUL, the Hoare-style logic reflects the stateful nature of \BiGUL\ programming, and is designed domain-specifically such that the programmer can work out the precise behaviour of \BiGUL\ programs with reasonable effort (rather than breaking the abstraction and working with the more messy underlying semantics).
...
\begin{acks}                            %% acks environment is optional
  ...
  %% extraction tools.
We would like to thank Jeremy Gibbons and Li Liu for commenting on an early draft of this paper, and Shin-Cheng Mu for the fruitful discussions during his visit at NII.
