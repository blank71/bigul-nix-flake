...
\documentclass[acmlarge,fleqn,anonymous]{acmart}\settopmatter{printfolios=true}
...
The theory underlying the Hoare-style logic has been formalised and checked in \Agda, but this paper presents the Hoare-style logic in a semi-formal way to make it easily understood and usable by the working \BiGUL\ programmer.\todo{Rewrite the abstract?}
...
in contrast to the original lenses, \BiGUL\ is designed to express |put| transformations, and gives the programmer unprecedented freedom to specify their intended update strategies directly.
However, due to the more intricate nature of \BiGUL's bidirectional semantics, it is still hard to guarantee that a \BiGUL\ program will exhibit the intended behaviour.\todo{No good to reason in terms of the underlying semantics}
...
Inevitably they have to enrich their programs with putback information to gain more control, but that makes constructing and understanding such programs more awkward, having to break the abstraction and reason in both directions in terms of the underlying semantics.\todo{Emphasise this point}\
...
\BiGUL\ and its Hoare-style logic make a nice example of this statement.\todo{Revise to emphasise difference from reasoning in terms of the underlying semantics}
