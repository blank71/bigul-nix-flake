...
One such framework is \citet{Foster-lenses}'s \emph{asymmetric lenses}, which are highly influential such that the term \emph{bidirectional programming} has become largely synonymous with lens-based approaches \citep[including lens combinators and bidirectionalisation; see, e.g.,][]{Foster-bidirectional-programming-approaches}.
...
\begin{notation}
Relations on types |A(SUB 1)|\,, |A(SUB 2)|\,, $\ldots,$ |A(SUB n)| are assigned the type |POWER(A(SUB 1) TIMES A(SUB 2) TIMES {-"\cdots\;"-} TIMES A(SUB n))|.
...
Alignment is a representative problem in bidirectional transformations.
...
The Hoare-style logic helps to clearly distinguish the two approaches for the first time --- it is possible to precisely reason about bidirectional behaviour purely in the putback direction, whereas it is unthinkable that the same effect can be achieved from the opposite direction.

\paragraph{Wasn't there already previous work on controlling the putback behaviour?}

To name a few, the ``fixup functions'' in \citet{Foster-lenses}'s ``general conditionals'', the parameters of \citet{Bohannon-relational-lenses-tech-report}'s $\mathtt{join\_template}$, and the alignment keywords like ``\texttt{key}'' and ``\texttt{best}'' in \name{Boomerang}~\citep{Bohannon-Boomerang,Barbosa-matching-lenses} are all constructs which appear in programs designed to look like forward transformations but are meaningful only in the putback direction.
Constructs with similar purposes can also be found in bidirectionalisation approaches, such as \citet{Voigtlander-shape-plug-ins}'s ``shape bidirectionaliser plug-ins''.
Apart from offering only limited and/or special-purpose customisation of putback behaviour, the fundamental problem with these languages is that their programs contain a mixture of forward and backward information, and to understand such programs, the only way is to reason in both directions and in terms of the complex underlying semantics.
...
\BiGUL\ is unique since it offers a successful abstraction in which bidirectional programs can be reasoned about unidirectionally, as clearly reflected in the Hoare-style logic.
