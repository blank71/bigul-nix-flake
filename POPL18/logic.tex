...
\end{code}
This example clarifies the misconception that the programmer needs to have a |get| in mind when programming |put| --- rather, what the putback-based programmer has in mind is a consistency relation (|COM(ss' v || (all(t)) t `elem` ss' => t = v)| in this case), which may be functional but need not be executable.
