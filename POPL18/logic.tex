...
Most of \BiGUL's expressive power stems from its |case| construct, which has gone beyond \citet{Foster-lenses}'s ``general conditional'' in terms of expressiveness, allowing, in particular, key-based list alignment to be implemented using only simple and general-purpose primitives for the first time.
\BiGUL\ has also been put to the test in practical scenarios~\citep{Colson-BiGUL-configuration, Zhu-BiYacc, Zhao-BiGUL-feature-models-use-cases,Kinoshita-bidirectional-certified-programming}.
...
\varparagraph{Are there additional examples of verified \BiGUL\ programs?}
...
replaceAll =
  ...
  ^  ^  ^    *  replaceAll
\end{code}
The following triples are verified for |replaceAll|:
\begin{code}
...
\end{code}
The range triple is particularly interesting as it has a nontrivial output range.
...
This will require better tool support than the current \Agda\ formalisation, which is not intended to be easy to use (and is, in fact, exceedingly tedious to work with).
Instead, we plan to adapt the Hoare-style logic for easier mechanisation, in particular automated theorem proving (perhaps in the style of \name{LiquidHaskell}~\citep{Vazou-LiquidHaskell}), so as to verify larger-scale bidirectional programs with reasonable effort.
