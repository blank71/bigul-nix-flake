...
Following some previous work which took exactly this \emph{putback-based} programming approach~\citep{Pacheco-putlenses, Pacheco-BiFluX, Hu-putback-validity}, \citet{Ko-BiGUL} proposed a language \BiGUL\ (Bidirectional Generic Update Language).
...
Moreover, since |put| uniquely determines |get| by well-behavedness (but not vice versa), the putback-based programmer is in full control of not only the putback direction but also the forward behaviour.
...
\varparagraph{How powerful is \BiGUL?}
Most of \BiGUL's expressive power stems from its |case| construct, which has gone beyond \citet{Foster-lenses}'s ``general conditional'' in terms of expressiveness, allowing, in particular, key-based list alignment to be implemented directly for the first time.
\BiGUL\ has also been put to the test in practical scenarios~\citep{Colson-BiGUL-configuration, Zhao-BiGUL-feature-models-use-cases}.
