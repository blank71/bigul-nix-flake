...
\todo[inline]{``In summary'' at the end of the theory of range triples}

\todo[inline]{display more things if possible}
...
For example, |ASSERT(COM(s v || s = v))| is usually abbreviated to |ASSERT(s v || s = v)|.
\end{notation}
...
This ``side effect'' normally does not prevent us from deriving range triples, though, since the preconditions we specify are normally larger than the consistency relations, which in turn contain the |get| functions.
...
Verifying the range of |alwaysResetHeight| is a more interesting example, where we will see how a non-trivial output range can be derived with the consequence rule:
\begin{code}
(ASSERTRANGE(_ _))
rearrV v -> (v, 0)
  (ASSERTRANGE(_ (_ , 0)))
...
\BiGUL\ is designed datatype-generically for inductive data structures.
...
For example, let us peek at the key-based alignment program |keyAlign| in \autoref{fig:keyAlign}, which we will verify in \autoref{sec:key-based-alignment}.
...
We will need to incorporate this size-based termination argument into our proof rule for recursive programs, and show that the rule is sound.
...
Another possible solution is to stay with inductive \BiGUL\ programs and introduce a ``constructive fixed-point'' which can decide, for every input, how many times the body of a fixed-point should be expanded, thereby circumventing the need for truly infinite programs.
...
Let |b : S <~ V| be a recursive program of the form |b = f b| where |f : (S <~ V) -> (S <~ V)| is the usual non-recursive function defining the body of~|b|, and suppose that we want to show $\models |(ASSERT R) b (ASSERT R')|$, i.e., the conclusion of \autoref{thm:putback-soundness} for some specific precondition~|R| and postcondition~|R'| --- we cannot hope to establish a putback triple for~|b|, since putback triples are defined for finite programs, whereas |b|~is infinite.
...
\end{code}
The basic proof idea is to prove something about~|f| like:
\begin{equation}
...
\end{align}
then:
\[ |(all(l, n)) ~ l >= n ~ =>| \quad |(ASSERT(R CAP (COM(s v || measure s v = n)))) ~ expand (suc l) f ~ (ASSERT R')| \]
\end{theorem}
\begin{proof}
Nested induction on |l|~and~|n|.
\end{proof}

The conclusion of \autoref{thm:putback-finite-expansion} says that a finite expansion of~|f| will compute successfully from~|R| to~|R'| if |f|~is expanded more times than the measured size of the input.
We can say this more directly with the following semantic statement.
\begin{corollary} \label{cor:putback-finite-expansion-semantics}
The conclusion of \autoref{thm:putback-finite-expansion} implies:
\[ |(all(l))| \quad \models |(ASSERT(R CAP (COM(s v || measure s v <= l)))) ~ expand (suc l) f ~ (ASSERT R')| \]
\end{corollary}

\autoref{cor:putback-finite-expansion-semantics} justifies the soundness of \ref{prop:PutbackRecursion} as a proof rule for recursive programs, since it implies that all inputs in~|R| can be dealt with when |f|~is expanded enough times and the outputs will fall into~|R'| as required.
The \ref{prop:PutbackRecursion} rule itself combines the basic proof idea~(\ref{prop:basic-proof-idea}) with the sized-based termination argument given in the beginning of this section: in the outermost precondition, the size of the input source and view is bound to a ghost variable~|n|, and we can make recursive invocations wherever the size~|m| of the current source and view is strictly less than~|n|.

Analogously for range triples, we have a \ref{prop:RangeRecursion} rule, whose soundness is justified by \autoref{thm:range-finite-expansion} and \autoref{cor:range-finite-expansion-semantics} below.
...
\end{align}
then:
\[ |(all(l, n)) ~ l >= n ~ =>| \quad |(ASSERTRANGE(R CAP (COM(s v || measure s v = n)))) ~ expand (suc l) f ~ (ASSERTRANGE(P' n))| \]
\end{theorem}

\begin{corollary} \label{cor:range-finite-expansion-semantics}
The conclusion of \autoref{thm:range-finite-expansion} implies:
\[ |(all(l))| \quad \models |(ASSERTRANGE(R CAP (COM(s v || measure s v <= l)))) ~ expand (suc l) f ~ (ASSERTRANGE(BIGCUP ~ [ P' n || n <= l ]))| \]
\end{corollary}

The \ref{prop:RangeRecursion} rule instructs us to derive an output range |P' n| that can depend on the ghost variable~|n| bound to the size of the input in the precondition.
(In the case of |keyAlign|, the range triple we will derive is |(ASSERTRANGE(_ vs || length vs = n)) keyAlign ... (ASSERTRANGE(ss || length ss = n))|.)
Recursive invocations can be made in the derivation, and the estimated output range for the recursive invocation is |P' m| where |m|~is the size of the current input, provided that |m|~is strictly less than~|n| --- otherwise, the estimated output range will be empty.
...
keyAlign ks kv b c =
  ...
  where
    ...
    extract ks kv v (s :: ss) = if ks s = kv v  then  (s, ss)
                                                      ...
                                                      in   (s', s :: ss')
\end{code}
\caption{Key-based alignment in \BiGUL}
\label{fig:keyAlign}
\end{figure}
...
Understanding the bidirectional behaviour of a \BiGUL\ program almost only involves understanding its putback behaviour.
A successful extension of \autoref{thm:dominance}.
