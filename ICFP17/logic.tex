...
A typical example is maintaining consistency among documents of different formats, where consistency means that the
documents have the same content (despite being in different formats); whenever the content of one document is changed, the other documents should also be updated to restore consistency.
...
Over the past decade, frameworks of \emph{bidirectional transformations}~(BXs) have been proposed by different communities to address various synchronisation problems~\citep{Czarnecki-BX}.
One influential framework from the programming language community is \citet{Foster-lenses}'s \emph{asymmetric lenses}, which are designed for synchronising two pieces of data where one side --- which is called the \emph{source} --- has more information than the other, which is called the \emph{view}.
The programmer writes a lens program to describe a forward |get| transformation that computes a consistent view from a source; whenever the source is changed, we can simply rerun |get| to produce a new consistent view.
The same lens program can also be interpreted as a backward |put| transformation that takes a source and a possibly modified view, and produces an updated source that is consistent with the view and can retain some information of the original source.

By construction, all lens programs satisfy two inverse-like \emph{well-behavedness} laws called \ref{prop:PutGet} and \ref{prop:GetPut} (\autoref{thm:well-behavedness}).
\citet[Section~4.4]{Stevens-QVT} provides a revealing perspective to understand these well-behavedness laws: the |get| transformation denoted by a lens can be regarded as defining a consistency relation between the source and view; \ref{prop:PutGet} then says that the |put| transformation correctly restores consistency, i.e., the updated source and the view always satisfy the consistency relation, and \ref{prop:GetPut} says that the |put| transformation performs no update if the input source and view are already consistent.
From this perspective, what \citet{Foster-lenses}'s lenses offer is a highly declarative programming model, in which the programmer only specifies a consistency relation (in terms of a |get| transformation) and obtains a consistency restorer (a |put| transformation) that is guaranteed (by well-behavedness) to respect the consistency relation but is otherwise arbitrary.
This is unsatisfactory in practice, since we care not only about consistency but also about how consistency restoration is performed, whereas with lenses it is difficult to figure out the latter.

To be concrete, let us consider a simple synchronisation problem where the source is a pair of numbers representing the width and height of a rectangle, and the view is the width --- that is, the source and view are consistent when the view is equal to the width of the rectangle.
With respect to this definition of consistency, there are a variety of sensible consistency restoration strategies: given a rectangle and a view, in addition to replacing the width with the view,
\begin{itemize}
\item we can always keep the height unchanged --- this is the typical ``least-change'' strategy that aims to perform as little update to the source as possible;
...
\item we can reset the height to zero if the view is different from the width --- in general this strategy is useful when the view side does not know whether or how the source side maintains its internal consistency, and thus simply chooses to invalidate associated data and leave them for the source side to update later;
...
\item we can use the height as a counter that is incremented every time an inconsistency is repaired --- this is somewhat bizarre for rectangle width updating, but may well be required for some other interpretations of the problem.
\end{itemize}
As we can see, even for a simple problem like rectangle width updating, there are already many possible update strategies, not to mention complex, real-world scenarios.
All the above update strategies restore the same consistency relation but have different \emph{retentive} behaviour --- that is, they retain the information of the original source in different ways.
The programmer must be empowered to reason about the retentive behaviour of their programs to be sure that the behaviour is suitable for their intended applications.

To capture the myriad possibilities of update strategies, what can be better than having languages for \emph{programming} such strategies?
Following a few other groups of authors~\citep{Pacheco-putlenses, Pacheco-BiFluX, Hu-putback-validity}, \citet{Ko-BiGUL} take exactly this approach, and propose a ``putback-based'' language \BiGUL\ (short for the \emph{Bidirectional Generic Update Language}).
Analogous to \citet{Foster-lenses}'s lenses, every \BiGUL\ program denotes a well-behaved pair of |put| and |get| transformations, and that well-behavedness has been formally verified in \Agda~\citep{Norell-thesis};
different from \citet{Foster-lenses}'s lenses, \BiGUL\ is designed for expressing |put| transformations, and gives the programmer unprecedented freedom to specify their intended update strategies directly.

This paper takes the putback-based approach one step further: building on \citeauthor{Ko-BiGUL}'s work, we propose a \emph{Hoare-style logic} that empowers the programmer to precisely reason about both the |put| and |get| behaviour of \BiGUL\ programs.
For example, the programmer can express the height-keeping and height-resetting strategies as two \BiGUL\ programs |keepHeight| and |resetHeight|, and with our Hoare-style logic, the programmer can prove two Hoare-style \emph{putback triples} to make sure that the two programs correctly restore the consistency and have the intended retentive behaviour:
\begin{code}
(ASSERT(_ _)) ~  keepHeight   ~ (ASSERT((w', h') (_  , h) v | w' = v && h' = h))
(ASSERT(_ _)) ~  resetHeight  ~ (ASSERT((w', h') (w  , h) v | w' = v && (w = v => h' = h) && (w /= v => h' = 0)))
\end{code}
These two putback triples state that both |keepHeight| and |resetHeight| work on any input pairs of source and view (due to the always true precondition |ASSERT(_ _)|) and will update the width to be equal to the view (|w' = v|), that |keepHeight| will retain the original height (|h' = h|), and that |resetHeight| will retain the height if the original width is equal to the view (|w = v => h' = h|) or reset the height otherwise (|w /= v => h' = 0|).
With just a bit more effort, the programmer can also prove that the |get| transformations denoted by these two programs work on any input rectangle and extract its width, conforming to the consistency relation (|w' = v|) stated in the above triples.

For the rest of this paper:
We first review the theory of asymmetric lenses and build a theory of putback triples in \autoref{sec:putback-triples}.
\autoref{sec:putback-proof-rules} then introduces the concrete putback proof rules along with \BiGUL's constructs, and gives some derivation examples, including those for |keepHeight| (\autoref{ex:keepHeight}) and |resetHeight| (\autoref{ex:resetHeight}).
\autoref{sec:range-triples} complements putback triples with another set of \emph{range triples} so that \BiGUL's bidirectional behaviour can be completely characterised.
\autoref{sec:recursion} further develops proof rules for recursive \BiGUL\ programs, and enables us to verify a \BiGUL\ implementation of key-based list alignment in \autoref{sec:key-based-alignment} as a showcase example.
Discussion and conclusion follow in Sections \ref{sec:discussion}~and~\ref{sec:conclusion}.

Everything in this paper from theorems to derivations has been formalised and checked in \Agda\ version~2.5.2 with standard library version~0.13, but this paper does not present the \Agda\ formalisation, which is provided only as supplementary material.
Instead, we focus on explaining the intuition, and present the Hoare-style logic in a semi-formal way to make it suitable for human reasoning.
\BiGUL\ is originally developed in \Agda\ and later also ported to \Haskell\ as an embedded language, and the syntax of \BiGUL\ used in this paper is a hypothetical one abstracted from the \Haskell\ port of \BiGUL.
The host language is total and can be thought of as \Agda\ under the imperfect disguise of \Haskell; in particular, we will use some standard \Haskell\ types and functions, and allow some ``superficial'' general recursion and partiality (which will be justified in Sections \ref{sec:recursion}~and~\ref{sec:key-based-alignment}).
...
\todo[inline]{Every \BiGUL\ program denotes a \emph{well-behaved lens}, which consists of two transformations inverse to each other in a certain sense (\autoref{thm:well-behavedness}).
...
The relation-level `|*|'~operator can (indeed) be seen as a simple variant of separating conjunction~\citep{Reynolds-separation-logic}, and can be defined arity-generically to construct a relation on $n$~pairs from an $n$-ary relation on all the first components and the other on all the second components.
...
The theorem is proved by nested induction on |l|~and~|n|.
...
Having the two recursion rules, we are now ready to verify |keyAlign|.\todo{\autoref{thm:total-forward-consistency} and other theorems also work semantically}
...
\end{itemize}
Due to space restrictions, we will not be able to go through the program and its verification in detail in this paper, so here we offer a quick overview instead.
...
The fourth branch is the most interesting: the head view element~|v| has a corresponding source element (which is not at the head position), so we |extract| the first source element with the same key as~|v| and put it at the head (intending to reenter the second branch after adaptation).
...
For simplicity, we assume that |b|~can compute successfully for any pair of source and view elements as long as they have the same key; also, |b|~should guarantee that the updated source and the view will have the same key, apart from any other postcondition |R' : (POWER(S TIMES S TIMES V))| it can establish.
As a putback triple:
\[ |(ASSERT(s v || ks s = kv v)) ~ b ~ (ASSERT(R' CAP (COM(s' _ v || ks s' = ks v))))| \]
We will abbreviate |R' CAP (COM(s' _ v || ks s' = ks v))| as~|T'|.
...
We can then derive:
\begin{code}
(all n) {-"\quad"-} (ASSERT(_ vs | length vs = n)) ~ keyAlign ks kv b c ~ (ASSERT(ss' ss vs | (some(~ ss'')) (STAR T') ss' ss'' vs && Retentive ss vs ss''))
\end{code}
...
This |Retentive| relation turns out to be slightly tricky to define, especially when we do not require that keys in a list are all unique; the definition we adopt says that if a key appears |n|~times in the view list, then the first |n|~elements with that key in the original source list will be retained.

We will only sketch the verification of the second normal branch and the second adaptive branch.
...
^  (ASSERT((s' :: ss') ss (v :: vs) | (some(~ ss'')) (STAR T') (s' :: ss') ss'' (v :: vs) && Retentive ss (v :: vs) ss'' && ks s' = kv v)) (ASSERTNUMBER *5)
\end{code}
We first look at how the \ref{prop:PutbackRecursion} rule is applied.
Assertion~1 is the actual precondition for the recursive invocation of |keyAlign ks kv b c|, and we rewrite it into assertion~2, saying that the length of the view list at this point is |pred n|, which is strictly less than~|n| since assertion~1 says that |n|~is a successor.
(The predecessor function is defined by |pred zero = zero| and |pred (suc n) = n|.)
The recursive invocation will thus succeed and establish assertion~3.
For the postcondition, the |rearrS|, |rearrV|, and product rules give us assertion~4, while we need to prove assertion~5.
...
^  \ ss (v :: _) -> extract ks kv v ss
^  (ASSERT(ss' ss (v :: vs) | (COM(ss' ss vs | (some(~ ss'')) (STAR T') ss' ss'' vs && Retentive ss vs ss'')) ss' (extract ks kv v ss) (v :: vs))) (ASSERTNUMBER *3)
^  (ASSERT(ss' ss vs | (some(~ ss'')) (STAR T') ss' ss'' vs && Retentive ss vs ss'')) (ASSERTNUMBER *4)
\end{code}
Assertion~1 (where we omit the negations of the previous main conditions) is sufficient to guarantee the reentry condition stated in assertion~2: the source list~|ss| must be nonempty so that |extract ks kv v ss| will successfully produce a nonempty list, whose head will then be guaranteed to have the same key as the head view element~|v|.
...
This is a good example showing that adaptation should be done cautiously to maintain sufficient links between the adapted source and the original source, or otherwise it can be immensely difficult to prove the implication from the adapted postcondition to the final postcondition.
...
If |T' SUBSETEQ (COM(s' _ v || C s' v))| for some element-level consistency relation |C : (POWER(S TIMES V))|, then we know that, for any source list, |get (keyAlign ks kv b c)| will successfully produce a view list of the same length, and each pair of source and view elements at the same position will satisfy~|C|.
For the actual derivation, again due to space restrictions we can only give a quick sketch:
...
Their union |COM(ss || length ss = n)| is the output range of the entire |case|.
...
\paragraph{How powerful is (the current) \BiGUL\ compared with previous bidirectional languages?}
It might be said that most of the expressive power of the current \BiGUL\ stems from its |case| construct, which has gone beyond \citet{Foster-lenses}'s ``general conditional'' in terms of expressiveness, allowing, in particular, key-based list alignment to be implemented directly.
The case analysis constructs in \citet{Ko-BiGUL}'s original \BiGUL\ were essentially the same as \citeauthor{Foster-lenses}'s conditionals, and \citeauthor{Ko-BiGUL} had to provide list alignment as an extra and complex primitive.

While we are on the subject of alignment, the reader might be interested in a comparison with \citet{Barbosa-matching-lenses}'s matching lenses.
Matching lenses offer some fairly sophisticated matching strategies that can be hard to implement nicely in \BiGUL\ so far, but they are special-purpose and require the invention of a new framework in which everything is built from scratch.
In contrast, \BiGUL\ is designed with the ultimate aim of expressing all lenses using just a fixed set of simple primitives, like what we can do in general-purpose programming languages --- hence the need for a general reasoning principle like the Hoare-style logic.

\paragraph{\BiGUL\ claims to be ``putback-based'' but is still a lens language. Is there really a fundamental difference between \BiGUL\ and previous ``|get|-based'' lens languages?}
When it comes to language definition, \BiGUL\ programs denote lenses and have to be defined in both directions, exactly the same as other lens languages.
...
Inevitably they have to add annotations to gain more control of the putback behaviour, but that makes constructing and understanding such programs more awkward, having to think in both directions.
In contrast, \BiGUL's putback-based design lets the programmer construct programs purely in the |put| direction.
The Hoare-style logic makes the distinction between the two approaches absolutely clear for the first time --- it is possible to reason about bidirectional programs purely in the putback direction, whereas it is unthinkable that the same effect can be achieved in the opposite direction.

\paragraph{Doesn't \autoref{thm:total-forward-consistency} say that we need to reason in both directions anyway?}

We have consistently explained how range triples can be derived by thinking in the putback direction.
Even if the sceptic reader insisted that deriving range triples requires thinking in the forward direction, they would still have to admit that it is much easier to prove that |get| is contained in the precondition of |put|, as required by \autoref{thm:total-forward-consistency}, than to prove that it is contained in the consistency relation.
...
\paragraph{What reasoning principles were there for bidirectional programming?}
...
\[ \begin{array}{l}
|(ASSERT (a b' || (some(b, c)) R a c && R' a b && U' b' b c)) ~ l ~ (ASSERT(COM(a' _ b || R' a' b) CAP T')| \\
...
\end{array} \]
and we have actually proved that the rule is sound, but the form of the rule is too complex and hardly usable.
We will need to find a sweet spot and design a composition rule that is perhaps not as general as the above one but can still say enough about the retentive behaviour; most importantly, this rule should give an instruction on how composition can be effectively used, and only with such a rule can composition be justified as a worthy construct.
It should be noted that while composition is included in other languages like \citet{Foster-lenses}'s, the problem with controlling the retentive behaviour of composition has always existed, as discussed by, e.g., \citet[Section~2.2]{Diskin-delta-asymmetric}.
...
More specifically, we have demonstrated how an adequate understanding of a \BiGUL\ program's bidirectional behaviour can be reduced to reasoning exclusively about its putback behaviour.

Bidirectional programming has been highly declarative, in the sense that the programmer writes down only a consistency specification (perhaps with some behavioural annotations) and relies on the system to produce a well-behaved but otherwise arbitrary implementation.
However, it has long been realised that declarative approaches are hardly enough for practical bidirectional applications \citep[see, e.g.,][Section~4.1]{Stevens-QVT}.
The BX community currently concentrates on the exploration of more forms of well-behavedness properties \citep[see, e.g.,][]{Cheney-least-surprise}, but we should not be satisfied with only well-behavedness guarantees.
Instead, we should also start aiming to precisely characterise the behaviour of bidirectional programs like what the \BiGUL\ programmer can now do with the Hoare-style logic, treating bidirectional programming seriously as we treat general-purpose programming.
...
In the case of \BiGUL, we have shown how the precision of the Hoare-style proof rules can complement the informal description of \BiGUL's semantics, and explained how and in what format semi-formal reasoning can be performed for and integrated into \BiGUL\ programs with reasonable effort.
