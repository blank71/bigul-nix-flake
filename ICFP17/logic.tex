...
In particular, |resetHeight| in \autoref{ex:resetHeight} can be seen as |emb fst reset| where |reset = \ _ v -> (v, 0)|.
...
alwaysResetHeight =  rearrV v -> (v, 0)
                       ...
                       *  replace
\end{code}
We can prove that the putback behaviour of |alwaysResetHeight| coincides with |reset| by deriving the putback triple |(ASSERT(_ _)) alwaysResetHeight (ASSERT(s' _ v || s' = (v, 0)))|.
By \autoref{thm:well-behavedness}, the |put| and |get| semantics of |alwaysResetHeight| must form a well-behaved pair, so we can actually achieve precise embedding of |reset|'s putback behaviour and well-behavedness at the same time and without introducing the extra logic of |emb|\kern1pt!
There is no free lunch though: |alwaysResetHeight| has a serious disadvantage compared to |emb fst reset|.
By \autoref{thm:partial-forward-consistency}, we have
\[ |(GRAPH(get (emb fst reset)) ~ SUBSETEQ ~ (COM((w, _) v || w = v))| \AND |(GRAPH(get alwaysResetHeight)) ~ SUBSETEQ ~ (COM((w, 0) v || w = v))| \]
so both |get (emb fst reset)| and |get alwaysResetHeight| behave like |fst|.
In fact, both inclusions can be strengthened to equalities, enabling us to distinguish |get (emb fst reset)|, which can compute successfully on all inputs, from |get alwaysResetHeight|, which fails to compute on inputs whose second component is non-zero.
However, we do not have the necessary machinery to strengthen the inclusions to equalities yet --- what is missing here is the ability to estimate the \emph{domain} of a forward transformation, i.e., the subset of sources on which the forward transformation can compute successfully.
...
The problem with |alwaysResetHeight| is now clear: it can only produce the sources whose second component is zero, so |get alwaysResetHeight| can compute successfully only on these sources.
...
where |R : (POWER(S TIMES V))| is the precondition (on the original source and the view) and |P' : (POWER S)| is the postcondition (on the updated source).
...
We can recover the intended interpretation of range triples by rewriting the right-hand side of \autoref{thm:range-soundness} into an equivalent statement primarily about |put|:
...
The right conjunct in \autoref{lem:range-interpretation} says that there is a ``side effect'' when we think about these triples in the putback direction: the subset of input pairs considered will be forced to include those related by |get| with its domain restricted to~|P'|.
...
That is, by supplementing a putback triple for~|b| with a range triple with the same precondition, we can know on which subset of sources |get b| will compute successfully and that the behaviour of |get b| conforms to the consistency relation.

Now we have enough machinery to tell us all we want to know about the bidirectional behaviour of a \BiGUL\ program:
...
\begin{example}[embedding pairs of transformations in \BiGUL]
Straightforward.
\begin{code}
(ASSERTRANGE(_ _))
case
...
alwaysResetHeight =
  ...
  rearrV v -> (v, 0)
    (ASSERTRANGE(_ (_ , 0)))
       ...
       replace
       (ASSERTRANGE(_))
    *  (ASSERTRANGE(_ 0)) (ASSERTRANGENUMBER *1)
       (ASSERTRANGE(h v | h = v)) (ASSERTRANGENUMBER *2)
       replace
       (ASSERTRANGE(_))
       (ASSERTRANGE(0)) (ASSERTRANGENUMBER *3)
    (ASSERTRANGE((_ , 0)))
  (ASSERTRANGE((_ , 0)))
\end{code}
\end{example}
