...
For simplicity, we assume that |b|~can run successfully for any pair of source and view elements as long as they have the same key; also, |b|~should guarantee that the updated source and the view will have the same key, apart from any other postcondition |R' : (POWER(S TIMES S TIMES V))| it can establish.
As a putback triple:
...
We can then derive:
\begin{code}
(ASSERT(_ _)) ~ keyAlign ks kv b c ~ (ASSERT(ss' ss vs | (some(~ ss'')) (STAR T') ss' ss'' vs && Retentive ss vs ss''))
\end{code}
where |(STAR T') : (POWER([S] TIMES [S] TIMES [V]))| is defined inductively by the following two rules:
\begin{code}
(STAR T')  []           []             []
(STAR T')  (s' :: ss')  (s'' :: ss'')  (v :: vs) ~ <== ~ T' s' s'' v &&  (STAR T') ss' ss'' vs
\end{code}
and |Retentive ss vs ss''| says that if a source element of~|ss| corresponds to a view element in~|vs|, then that source element will be in~|ss''|.
This ``correspondence'' turns out to be tricky to define.
...
^  ^  ^  ^  b
^  ^  ^  ^  (ASSERT T')
^  ^  ^  *  (ASSERT(_ vs | 1 + length vs = n))
^  ^  ^  ^  (ASSERT(_ vs | length vs = pred n && pred n < n))
^  ^  ^  ^  keyAlign ks kv b c
^  ^  ^  ^  (ASSERT(ss' ss vs | (some(~ ss'')) (STAR T') ss' ss'' vs && Keep ss vs ss''))
^  (ASSERT((s' :: ss') (s :: ss) (v :: vs) | T' s' s v && (some(~ ss'')) (STAR T') ss' ss'' vs && Keep ss vs ss''))
^  (ASSERT((s' :: ss') ss (v :: vs) | (some(~ ss'')) (STAR T') (s' :: ss') ss'' (v :: vs) && Keep ss (v :: vs) ss'' && ks s' = kv v))
\end{code}

|Keep ss vs ss'' && ks s = kv v ~ => ~ Keep (s :: ss) (v :: vs) (s :: ss'')|
...
keyAlign ks kv b c =
   ...
   ^  adaptive _ (_ :: _)
   ^  ^  \ ss (v :: _) -> c v :: ss
   (ASSERT(ss' ss vs | (some(~ ss'')) (STAR T') ss' ss'' vs && (all(i)) Keep i ss vs  => s !! i `elem` ss''))
\end{code}
...
\end{quote}
This fact can be observed, for example, in \citet{Foster-lenses}'s Figure~8, which clearly shows that their lens programs are supposed to be constructed like writing |get|; for a second example, \citet{Bohannon-relational-lenses}'s relational lenses are written like database queries, which are |get| transformations.
...
The Hoare-style logic makes the distinction between the two approaches absolutely clear for the first time --- it is possible to reason about bidirectional programs purely in the putback direction, whereas the same effect can never be achieved in the opposite direction.
