...
%format Domain = "\awa{\mathcal N}{\mathcal D}"
...
\end{array} \\
& \quad |where|\;|Domain = BIGCUP ~ [ M || (normal OR adaptive M ...) `elem` branches ]| \\[-1ex]
...
\begin{lemma}[finite expansion of putback triples]
Let |f : (S <~ V) -> (S <~ V)| and |measure : S -> V -> Nat|.
If, for all |n : Nat| and |rec : S <~ V|,
\[ |((all m) (ASSERT(R CAP (COM(s v || measure s v = m && m < n)))) ~ rec ~ (ASSERT R'))| \quad|=>|\quad |(ASSERT(R CAP (COM(s v || measure s v = n)))) ~ f rec ~ (ASSERT R')| \]
then, for all |l : Nat| and |n : Nat| such that |n <= l|,
\[ |(ASSERT(R CAP (COM(s v || measure s v = n)))) ~ expand (suc l) f ~ (ASSERT R')| \]
\end{lemma}

\begin{lemma}
\[ |(ASSERT R) ~ b ~ (ASSERT R')| \quad|&&|\quad |(ASSERT T) ~ b ~ (ASSERT T')| \quad|=>|\quad |(ASSERT(R CUP T)) ~ b ~ (ASSERT(R' CUP T'))| \]
\end{lemma}

\begin{corollary} For all |l : Nat|,
\[ |(ASSERT(R CAP (COM(s v || measure s v <= l)))) ~ expand (suc l) f ~ (ASSERT R')| \]
\end{corollary}

\begin{lemma}
Let |(VEC R) : Nat -> (POWER(S TIMES V))|, |(VEC b) : Nat -> (S <~ V)|, and |(VEC R') : Nat -> (POWER(S TIMES S TIMES V))| be nondecreasing chains.
\[ |((all(i)) (ASSERT(VEC R i)) ~ (VEC b i) ~ (ASSERT(VEC R' i)))| \quad|=>|\quad |(ASSERT(BIGCUP(VEC R))) ~ LUB(VEC b) ~ (ASSERT(BIGCUP(VEC R')))| \]
\end{lemma}

\begin{corollary}
\[ |(ASSERT R) ~ LUB(\ l -> expand (suc l) f) ~ (ASSERT R')| \]
\end{corollary}
\begin{proof}
|(\ l -> R CUP (COM(s v || measure s v <= l)))| is a nondecreasing chain with~|R| as its least upper bound.
\end{proof}

\begin{theorem}[finite expansion of range triples]
Let |f : (S <~ V) -> (S <~ V)|, |measure : S -> V -> Nat|, and |P : Nat -> (POWER S)|.
If, for all |n : Nat| and |rec : S <~ V|,
\[ |((all m) (ASSERTRANGE(R CAP (COM(s v || measure s v = m)))) ~ rec ~ (ASSERTRANGE(P m CAP (COM(_ || m < n)))))| \quad|=>|\quad |(ASSERTRANGE(R CAP (COM(s v || measure s v = n)))) ~ f rec ~ (ASSERTRANGE(P n))| \]
then, for all |l : Nat| and |n : Nat| such that |n <= l|,
