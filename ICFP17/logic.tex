...
There is, of course, no free lunch:  |alwaysResetHeight| has a serious disadvantage compared to |emb fst reset|.
By \autoref{thm:partial-forward-consistency}, we have
\[ |(GRAPH(get (emb fst reset)) ~ SUBSETEQ ~ (COM((w, _) v || w = v))| \AND |(GRAPH(get alwaysResetHeight)) ~ SUBSETEQ ~ (COM((w, 0) v || w = v))| \PERIOD \]
In fact, both inclusions can be strengthened to equalities, so |get alwaysResetHeight| has a strictly smaller domain (which is the subset of inputs that a potentially partial transformation can successfully map to a result).
However, we do not have the necessary machinery to strengthen the inclusions to equalities yet.
More generally, as noted below \autoref{thm:partial-forward-consistency}, we still need to develop an additional piece of machinery so that we can know more about forward behaviour.
