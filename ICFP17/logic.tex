...
Among the frameworks of bidirectional transformations proposed for addressing various synchronisation problems, \citeauthor{Foster-lenses}'s lenses have influenced the design of a generation of bidirectional programming languages.
...
However, synchronisation problems are diverse and require vastly different consistency restoration strategies, and the programmer must be empowered to fully control and reason about the consistency restoration behaviour of their bidirectional programs.
The putback-based approach to bidirectional programming was proposed to address this issue once and for all, and this paper takes the approach one step further by proposing a Hoare-style logic for \citeauthor{Ko-BiGUL}'s putback-based language \BiGUL.
With this Hoare-style logic, the \BiGUL\ programmer can precisely understand the bidirectional behaviour of their programs by reasoning solely in the putback direction.
The theory underlying the Hoare-style logic has been formalised and checked in \Agda, but this paper gives a semi-formal presentation of the Hoare-style logic to make it easily understood and usable by the working \BiGUL\ programmer.
...
A typical example is synchronisation among documents of different formats, in which case consistency means that the
documents have the same content; whenever the content of one document is changed, the other documents should also be updated to restore the consistency.
...
Over the past decade, frameworks of \emph{bidirectional transformations} have been proposed by different communities to address a diverse range of synchronisation problems~\citep{Czarnecki-BX}.
...
This is unsatisfactory in practice, since we care not only about consistency but also about how consistency restoration is performed, whereas with lenses it is difficult to understand or control the latter.
...
\item we can reset the height to zero if the view is different from the width --- in general this strategy is used when the view side does not know whether or how the source side maintains its internal consistency, and thus simply chooses to invalidate associated data and leave them for the source side to update later;
...
All the above update strategies restore the same consistency but have different \emph{retentive} behaviour --- the way in which the information of the original source is retained --- dealing with different needs.
The programmer must be empowered to fully control and reason about the retentive behaviour of their programs to be sure that the behaviour is suitable for the intended applications.

Seeing that there are a myriad possibilities of update strategies, what can be better than having languages for \emph{programming} such strategies, capturing the myriad possibilities once and for all?
Following some previous work~\citep{Pacheco-putlenses, Pacheco-BiFluX, Hu-putback-validity} on this putback-based programming approach, \citet{Ko-BiGUL} proposed a language \BiGUL\ (short for the \emph{Bidirectional Generic Update Language}).
Like the original lenses, every \BiGUL\ program denotes a well-behaved pair of |put| and |get| transformations;
different from the original lenses, \BiGUL\ is designed for expressing |put| transformations, and gives the programmer unprecedented freedom to specify their intended update strategies directly.
...
This paper takes the putback-based approach one step further: building on a revised version of \BiGUL, we propose a \emph{Hoare-style logic} that empowers the programmer to precisely reason about both the |put| and |get| behaviour of \BiGUL\ programs.
...
After recapping some facts about asymmetric lenses in \autoref{sec:asymmetric-lenses}, we will build a theory of putback triples in \autoref{sec:putback-triples}.
...
\autoref{sec:range-triples} will complement putback triples with another set of \emph{range triples} so as to gain a better understanding of the |get| behaviour of \BiGUL\ programs.
...
Everything in this paper from theorems to derivation examples has been formalised and checked in \Agda\ version~2.5.2 with standard library version~0.13, but this paper will not present the \Agda\ formalisation, which is provided only as supplementary material.
Instead, we will focus on explaining the intuition, and present the Hoare-style logic in a semi-formal way to make it suitable for human reasoning.
\BiGUL\ is originally developed in \Agda\ and also ported to \Haskell\ as an embedded language.
The syntax of \BiGUL\ used in this paper is a hypothetical one abstracted from the \Haskell\ port of \BiGUL.
The host language is total and can be thought of as \Agda\ imperfectly disguised as \Haskell; in particular, we will use some standard \Haskell\ types and functions, and allow some ``superficial'' general recursion and partiality (which will be justified in Sections \ref{sec:recursion}~and~\ref{sec:key-based-alignment}).
...
We start from a brief recap of some general facts about asymmetric lenses, and state these facts directly in terms of \BiGUL.
...
\autoref{def:put-and-get} and \autoref{thm:well-behavedness} below are provided by \citet{Ko-BiGUL} for the original \BiGUL, whereas this paper uses a revised version of \BiGUL; accordingly, we have revised the internals of the definition and theorem, whose statements remain the same though.

\begin{definition} \label{def:put-and-get}
A \BiGUL\ program~|b| operating on source type~|S| and view type~|V| is assigned the type |S <~ V|, and has two semantics:
\begin{code}
put  b : S -> V ->  Maybe S
get  b : S ->       Maybe V
\end{code}
...
As noted by \citeauthor{Ko-BiGUL}, the two semantics in \autoref{def:put-and-get} are potentially partial computations modelled explicitly as total |Maybe|-computations.
...
Well-behavedness implies the following theorem (originally \citet{Foster-thesis}'s Lemma~2.2.5), which says that once a putback transformation is specified, the corresponding forward transformation is uniquely determined.
...
On top of \BiGUL's semantics (\autoref{def:put-and-get}), we will introduce an axiomatic system of Hoare-style triples for saying when \BiGUL\ programs (as putback transformations) can compute successfully and return results satisfying the specified properties.
Before doing so, let us first fix our notation of relations (for specifying preconditions and postconditions).
...
We omit the proof of this theorem, which is by induction on the proof rules as usual.
...
In this section we will introduce the putback proof rules for \BiGUL\ shown in \autoref{fig:putback-proof-rules}, taking for granted that the theory in \autoref{sec:putback-triples} (in particular \autoref{thm:putback-soundness}) has been fully established (which is done by the accompanying \Agda\ formalisation).
...
The Hoare-style logic makes the distinction between the two approaches absolutely clear for the first time --- it is possible to reason about bidirectional programs purely in the putback direction, whereas it is unthinkable that the same effect can be achieved in the opposite direction, which is perhaps the reason that no similar reasoning principles have been proposed for |get|-based languages.
...
The bidirectional transformations community currently concentrates on the exploration of more forms of well-behavedness properties \citep[see, e.g.,][]{Cheney-least-surprise}, but we should not be satisfied with only well-behavedness guarantees.
...
In the case of \BiGUL, we have shown how the precision of the Hoare-style proof rules can complement the informal description of \BiGUL's semantics, and explained how and in what format semi-formal domain-specific reasoning can be performed naturally for \BiGUL\ programs.
