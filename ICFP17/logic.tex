...
\qquad \awa{\text{\emph{postcondition}\kern.5pt:\ \ }}{\text{\color{putback}\emph{reentry}\kern.5pt:\ \ }} |R (f s v) v ~ && ~ (NORM N) (f s v) v| \quad|&&|
\\
\qquad \text{\color{putback}\emph{postcondition}\kern.5pt:\ \ } |(all(s')) ~ R' s' (f s v) v ~ => ~ R' s' s v| \\ \hline
|(ASSERT(R CAP N)) ~ case branches ~ (ASSERT R')|
\end{array} \\
& \quad |where|\;|N = BIGCUP ~ [ M || (normal M exit E INNER b) `elem` branches ]| \\[-1ex]
& \quad \awa{|where|}{}\; |(NORM N) = BIGCUP ~ [ (NORM M) || (normal M exit E INNER b) `elem` branches ]|
\end{align*}
...
case
  ...
  (ASSERT'(coverage)(COM((w, _) v | w = v) CUP (COM(_ _)) ))
...
\end{array} \\
& \quad |where P' = BIGCUP ~ [ P'_n || n = (normal M exit E INNER b) `elem` branches ]|
\end{align*}

\begin{lemma}
\[ |(ASSERT R) ~ b ~ (ASSERT(s' _ _ || Q s')) (ASSERTRANGE P)| \quad|=>|\quad |P SUBSETEQ Q| \]
\end{lemma}
