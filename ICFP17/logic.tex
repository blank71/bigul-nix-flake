...
But that is not the whole story --- in fact, we can distinguish |get (emb fst reset)| from |get alwaysResetHeight| because the former can compute successfully on all inputs, whereas the latter can only compute successfully on inputs whose second component is zero, and is usually not what one wants in practice.
...
\autoref{thm:total-forward-consistency} tells us that, by supplementing a putback triple for~|b| with a range triple with the same precondition, we can know on which subset of sources |get b| will compute successfully and that the behaviour of |get b| conforms to the consistency relation.
...
If |(ASSERTRANGE R) b (ASSERTRANGE P')| has been established, meaning that the inputs in~|R| can induce everything in~|P'| through the execution of~|b|, then a larger input range~|T| can of course still induce everything in~|P'|, or indeed everything in any output range~|Q'| smaller than~|P'|.
...
Verifying the range of |alwaysResetHeight| is a more interesting example, since we will see how a non-trivial output range can be derived with the consequence rule:
\begin{code}
(ASSERTRANGE(_ _))
rearrV v -> (v, 0)
  (ASSERTRANGE(_ (_ , 0)))
...
(ASSERTRANGE((_ , 0)))
\end{code}
The interesting part is the second |replace|, for which we first establish the input and output ranges as assertions 1~and~2, as dictated by the |replace| rule.
...
Logically, assertions 1~and~4 indeed imply assertion~3, adhering to the consequence rule.
\end{example}

\section{Recursion}
\label{sec:recursion}

In this paper we will develop just enough theory to justify our rule for reasoning about recursive programs, and refrain from delving into coinductiveness or constructive continuity.
