...
CaseDomain []             = ∅
CaseDomain ((p , _) ∷ bs) = (True ∘ uncurry p) ∪ ((False ∘ uncurry p) ∩ CaseDomain bs)

NormalCaseDomain : List Branch → ℙ (S × V)
NormalCaseDomain []                      = ∅
NormalCaseDomain ((p , normal l q) ∷ bs) = (True ∘ uncurry p) ∪ ((False ∘ uncurry p) ∩ NormalCaseDomain bs)
NormalCaseDomain ((p , adaptive f) ∷ bs) = (False ∘ uncurry p) ∩ NormalCaseDomain bs

OutOfDomain : List Branch → ℙ (S × V)
OutOfDomain []             = Π
OutOfDomain ((p , b) ∷ bs) = (False ∘ uncurry p) ∩ OutOfDomain bs

OutOfRange : List Branch → ℙ S
OutOfRange []             = Π
OutOfRange ((p , b) ∷ bs) = elimBranchType (λ l q → False ∘ q) (λ f → Π) b ∩ OutOfRange bs

RangeDisjoint : List Branch → List Branch → Set
RangeDisjoint []             bs' = ⊤
RangeDisjoint ((p , b) ∷ bs) bs' = elimBranchType (λ l q → (True ∘ q) ⊆ OutOfRange bs') (λ f → ⊤) b ×
