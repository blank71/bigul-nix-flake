...
but also completely formally verified in the dependently typed programming language \Agda\ to guarantee that any putback transformation 
...
Despite the advantages of full control and unambiguity of putback-based BXs, languages for writing |put| (such as \BiFluX)
...
and is completely\todo{used with its everyday meaning}\ formally verified to guarantee that any |put| specified in the language is well-behaved.
...
\item Second, the well-behavedness of |BiGUL| has been completely formally verified using the dependently typed programming language \Agda~\cite{Norell-thesis,Nore08}\todo{\Agda\ version used}, so now we can fully confidently guarantee that all the inserted dynamic checks can indeed help to establish well-behavedness.
...
Our partial transformations are actually total functions\todo{|Maybe|-functions are merely the starting point}\ which wrap their results in the standard |^^^Maybe| datatype whose two constructors are |^^^just : A -> Maybe A| and |^^^nothing  : Maybe A| --- the result of a successful computation is wrapped in the |just| constructor, while failed computation is represented by |nothing|.
...
This approach does not work too satisfactorily\todo{why not}, though, when we consider the kind of proofs we need to construct for these programs:
When proving |PutGet| and |GetPut|, the antecedent is a proof that a program --- consisting of sub-programs bound together by the combinators --- computes successfully, and we need to analyse it to proofs saying that each of the sub-programs computes successfully, before these proofs are reassembled to show that a related composite program also computes successfully.
...
We thus define a datatype |Par|\todo{not a monad by itself}, which is a deep embedding of the operators that we use to write partial transformations:
\begin{code}
...
\end{code}
For convenience, there are also other deeply embedded operators for error-handling and assertion, which we omit from the presentation.
...
fail        ↦ x   =  ⊥
\end{code}
In prose: |return x| computes to~|x'| when |x|~is exactly~|x'|; |mx >>= f| computes to~|y| when there exists a value~|x|\todo{meaning of the underscore here}\ such that |mx|~computes to~|x| and |f x| computes to~|y|; and it is impossible for |fail| to compute successfully.
...
l ↔ r = record
  {    put  =  λ a c → Lens.get l a >>=
                 λ b → Lens.put r b c >>= Lens.put l a
...
\autoref{fig:BiGUL} shows (a simplified version of) the top-level definition of |BiGUL|.\todo{some of the auxiliary definitions will appear later}\
...
data ^^^BiGUL : U → U → Set₁ where
  ...
  ^^^align    :  (HIDE({S V : U} →))  (source-condition : ⟦ S ⟧ → Par Bool)
                                      ...
                                      BiGUL (list S) (list V)
  ^^^update   :  (HIDE({S : U} →))    (pat : Pattern S) (bs : BiGULs pat) →
                                      BiGUL S (Views' pat bs)
...
The types of the |BiGUL| constructors are indexed by the source and view types expected by the operations represented by the constructors, and ultimately we can write a well-typed interpreter to |Lens|:
\begin{code}
...
Due to space limitation, in this paper we will only provide a sketch of the lens semantics of |BiGUL|.
...
We will not present the full definitions of |interp| nor the auxiliary types used in \autoref{fig:BiGUL}, which are actually straightforwardly derived from the lens semantics.
...
Its lens semantics can in fact be derived from an \emph{isomorphism}\todo{clarify ``partial'' isomorphisms}:
...
\end{code}
Note that the view type is specified as~|^^^⊤|, which has one single inhabitant~|^^^tt| --- had the view type been a more complex type, there would have been no way for |get| to decide what to return.
...
The exact details of the aligning algorithm are pretty delicate (especially the last part for merging the sources that satisfy |source-condition| with those that do not), and these details being formally verified makes us absolutely confident that the algorithm is correct.\todo{possibility of factoring |align| into smaller components}
...
We can now define a family of types |^^^Pattern : U -> Set| such that, for any |D : U|, the type |Pattern D| contains exactly those patterns sensible for |⟦ D ⟧| --- for example, on all types we can use the variable pattern |^^^var : (HIDE({D : U} ->)) Pattern D| which matches anything, while on sum types we can use the patterns matching |inj₁| and |inj₂| constructors but not the pair pattern |^^^prod : (HIDE({D E : U} ->)) Pattern D ->| |Pattern E -> Pattern (D ⊗ E)|, which only makes sense for product types.
...
\end{code}
Given a pattern |pat| and a collection of lenses |ls : Lenses pat| at its variable positions, we can define a lens between |PatResult pat| and a type |Views pat ls| which is defined as a product of the view types specified in~|ls|:
\begin{code}
...
%\end{code}
If a given bunch of |paths : Paths pat pat'| use up all components of the old view, i.e., all possible paths for |pat| are contained in |paths|, then |pat|, |pat'|, and |paths| together specify a computation that can be automatically inverted --- since all components of the old view are present somewhere in the new view, we can always reassemble the old view from the new view (unless there is inconsistency --- a component of the old view might be copied into more than one positions of the new view, and the values at these positions of the new view must be the same when doing the reassembling).
...
\end{code}
where |^^^Invertible pat pat' paths| is defined to state that |paths| can pass a check for ensuring that all components of the old view are used.
...
Thus, in the definition of |CaseVBranch| below\todo{morally just |Lens S V|?}, we use a partial isomorphism on the view type --- e.g., an instance of |pat-iso| for doing pattern matching --- which converts it to a possibly less informative type, instead of just a decidable predicate as in |CaseSBranch|:
\begin{code}
...
Below we conclude this paper by contrasting putback-based BXs with |get|-based BXs, discussing issues about dynamic checks, comparing |BiGUL| with its closest relative \Putlenses, and talking briefly about porting |BiGUL| to \Haskell.
...
\todo[inline]{\cite{Grohne-formalizing-semantic-bidirectionalization}}
