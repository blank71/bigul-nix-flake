...
data ^^^BiGUL : U → U → Set₁ where
  ...
  ^^^align    :  (HIDE({S V : U} →))  (source-condition : ⟦ S ⟧ → Par Bool)
                                      ...
                                      BiGUL (list S) (list V)
  ^^^update   :  (HIDE({S : U} →))    (pat : Pattern S) (bs : Lenses pat) →
...
\end{code}
(The deep embedding into |BiGUL|'s |caseV| constructor restricts the isomorphisms to those induced by pattern matching (to be introduced in \autoref{sec:source-update}) --- a deeply embedded branch of type |^^^CaseVBranch' S V| is a pair of a pattern and a lens that updates a source of type~|S| with the result of decomposing a view of type~|V| using the pattern.)
...
They require |Par| to be extended with one more constructor
\begin{code}
...
That is, the ranges of (the |put| semantics of) the branches have to be disjoint in order for |put| to compute successfully.
...
It is probably worth noting that the well-behavedness proofs in |align-lens| are more difficult, but it is mostly due to the inherent difficulty of the proofs --- there is little overhead regarding the formalisation per~se, due to the nice structure of the definition of well-behavedness (in terms of~|_↦_|).
...
An indispensable operation is decomposing a source and updating its parts --- in lines 5--6 of \autoref{fig:bookstore}, for example, we need to decompose a source entry and then update its title and price.
...
The lens semantics we assign to |rearr| is then
\begin{code}
interp (rearr pat pat' paths) =
  interp b c' ↔ iso-lens (view-rearr-iso vpat vpat' paths ABBRV)
\end{code}
Note that we do not specify the invertibility proof above --- to do that we need to extend |interp| with an additional argument consisting of invertibility proofs for all |rearr| operations appearing in the program being interpreted.
This extension is omitted from the presentation for simplicity.
...
An unmatched |Office| entry means that a new employee is inserted, so we create a temporary |Employee| entry in the source list, which is then updated by |relocate-employee| using the unmatched |Office| entry.
(We put the view location in the temporary |Employee| entry merely for efficiency, as |relocate-employee| would not have to deal with mismatching source and view locations.)
...
relocate-to-UK = caseS
  (  (inUK  , normal    (update  (prod  unit (left    var))
                                 ((_ ,  skip) , (_ ,  replace)))) ∷
...
relocate-to-US = caseS
  (  (inUS  , normal    (update  (prod  unit (right   var))
                                 ((_ ,  skip) , (_ ,  replace)))) ∷
