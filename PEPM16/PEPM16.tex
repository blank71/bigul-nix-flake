...
\preprintfooter{RC1}   % 'preprint' option specified.
...
This paper is typeset with a modified version of lhs2\TeX\ such that all global \Agda\ identifiers are hyperlinked to their definitions, which should be helpful to those who read this paper electronically.
...
The source is a list of book data of type |String × String × ℕ × ℕ × Bool| representing a book's title, author, publication year, price, and whether it is in stock, and we intend to update the price of those books published in 2016 which are still in stock.
...
This additional argument is omitted from the presentation for clarity.
...
align-lens : (HIDE({S V : Set} ->))
  ...
  Lens (List S) (List V)
\end{code}
To explain its |put| and |get| semantics more concretely, below we will use the program in \autoref{fig:bookstore} to put the view list
\begin{code}
...
\end{code}
The |put| semantics of |align-lens| is as follows:
\begin{itemize}[leftmargin=*]
\item
...
We would like to thank Li Liu, Hugo Pacheco, and the anonymous reviewers of APLAS\,'15 and PEPM\,'16 for their valuable comments.
