...
Recently there have been investigations into the feasibility of writing bidirectional programs by describing only their putback directions~\cite{Hu-putback-validity, Pacheco-putlenses, Pacheco-BiFluX}.
...
The combinator library \Putlenses~\cite{Pacheco-putlenses} follows this approach, providing a set of putback-based combinators for writing complex BX programs.
Based on \Putlenses\ and the functional XML update language \textsc{Flux}~\cite{Cheney-FLUX}, a bidirectional XML update language~\BiFluX~\cite{Pacheco-BiFluX} is also designed and implemented.
...
\item Second, the well-behavedness of |BiGUL| has been completely formally verified using the dependently typed programming language \Agda~\cite{Norell-thesis,Nore08} (version 2.4.3 commit \texttt{82161c7}, with standard library version 0.9 commit \texttt{ed2494a}), so now we can fully confidently guarantee that all the inserted dynamic checks can indeed help to establish well-behavedness.
...
The kind of partial transformations we need are actually total functions which wrap their results in the standard |^^^Maybe| datatype whose two constructors are |^^^just : A -> Maybe A| and |^^^nothing  : Maybe A| --- the result of a successful computation is wrapped in the |just| constructor, while failed computation is represented by |nothing|.
...
When proving |PutGet| and |GetPut|, the antecedent is a proof that a program --- consisting of sub-programs bound together by the combinators --- computes successfully, and we need to analyse it to proofs affirming that each of the sub-programs computes successfully, before these proofs are reassembled to show that a related composite program also computes successfully.
...
Before moving on to |BiGUL|, to emphasise the advantage of using |Par|, let us consider how well-behavedness proofs would be constructed if we write partial transformations simply as |Maybe|-programs and state well-behavedness in terms of \Agda's equality type~|_≡_|\kern1pt.
