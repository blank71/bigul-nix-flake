...
when defining the semantics of |BiGUL| (more specifically, for view case analysis in \autoref{sec:view-case}), we need to write programs that produce some result or fail respectively when a sub-program fails or produces some result, but this is not possible in the setting of complete partial orders.
...
(An exception is |update|, the development for which deviates slightly from this process for some technical reason.)
...
For the rest of the operations we will only give a sketch for brevity.\todo{why deep embedding}


\subsection{Replacing, failure, and skipping}
\label{sec:basic-lenses}

The three most basic operations are |replace|, |failB|, and |skip|.\todo{|put| and |get| semantics}\
The |replace| operation is applicable when the source and view are of the same type, and its semantics is discarding the original source and returning the view as the updated source.
...
As mentioned at the end of \autoref{sec:basic-lenses}, we must ensure that view information is completely embedded into the source, and that requires more conscious management of view information.
...
That is, the ranges of (the |put| semantics of) the branches have to be disjoint in order for |put| to compute successfully.\todo{How is this conclusion arrived at?}
...
An indispensable operation is decomposing a source and updating its parts.
...
\end{code}
The types that we can encode within~|U| are those expressible in terms of atomic types, |⊤|, |_⊎_|\ucomma |_×_|\ucomma and |List|; applying |⟦_⟧| to an inhabitant of the universe decodes it to the type it represents.
...
\end{code}
The |update| constructor of |BiGUL| then takes a pattern~|p| and a bunch of |BiGUL| statements |bs : BiGULs p| at its variable positions.
...
Finally, the lens semantics we assign to |rearr| is
\begin{code}
...
If it is the US, we should adapt the source by changing the salary from dollars to pounds (by a function |^^^toPounds : Salary -> Salary| defined elsewhere) and change the location to an empty one in the UK, which will be replaced by the view location in the second round of the source case analysis following the adaptation.
...
We should now check whether all constraints for passing dynamic checking are met, working from the inner statements towards the outer ones.
...
For the |caseV| in |adjust-salary-and-office|, the ranges of |relocate-to-UK| and |relocate-to-US| are disjoint\todo{revise to conform with the end of the |caseV| section}\ since they respectively produce UK and US locations.
