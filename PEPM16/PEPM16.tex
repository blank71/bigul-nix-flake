...
\item Most importantly, |BiGUL| and its well-behavedness has been completely formally modelled and verified using the dependently typed programming language \Agda~\cite{Norell-thesis,Nore08}.
...
\end{code}
which is apparently more heavyweight than the $\lambda$-expression~\autoref{lambda-expression}, even for this small lens composition example.
...
(See \autoref{sec:concluding-remarks} for a comparison with Foster~et~al's general conditional lens~\cite[Section~6.3]{Foster-lenses}.)
...
\todo[inline]{revise to correspond to caseS}
...
This makes guaranteeing |PutGet| difficult, as we cannot easily guarantee that executing |put| followed by |get| will go through the same branch.
...
align-lens : (HIDE({S V : Set} ->))
  ...
  Lens (List S) (List V)
\end{code}
As an example, we use the program in \autoref{fig:bookstore} to put the view list
\begin{code}
("Pottery" , 850) ∷ ("Hiking" , 550) ∷ []
\end{code}
into the source list
\begin{code}
("Pottery"  , "Rowling"  , 2015  , 950  , true  ) ∷
("Habits"   , "Tolkien"  , 1954  , 450  , true  ) ∷
("Nanny"    , "Lewis"    , 2015  , 650  , true  ) ∷ []
\end{code}
The |put| semantics is as follows:
\begin{itemize}
\item
...
\begin{code}
("Hiking" , "(to be updated)" , 2015 , 550 , true)
\end{code}
in the updated source list.
\item
...
\begin{code}
("Nanny"    , "Lewis"            , 2015  , 650  , false  ) ∷
("Habits"   , "Tolkien"          , 1954  , 450  , true   ) ∷
("Pottery"  , "Rowling"          , 2015  , 850  , true   ) ∷
("Hiking"   , "(to be updated)"  , 2015  , 550  , true   ) ∷ []
\end{code}
\end{itemize}
...
The program in \autoref{fig:bookstore} is written such that these checks will be successful:
...
\todo[inline]{add view rearrangement}
...
We admit, though, that the expressive power of the current |BiGUL| is not obviously stronger than existing lenses\todo{``hard to understand as a scientific argument'' we can now say more explicitly that |BiGUL| does not offer additional power\ldots}\ (although the adaptive |put| semantics of |caseS| has shown some potential of putback-based design), and it might be hard to find some formal properties to differentiate the putback-based approach in this paper from other approaches to lenses.
...
\todo[inline]{more primitive constructs (especially alignment); |caseS| and |caseV| vs Foster~et~al's general conditional lens}
...
\todo[inline]{Deep embedding because we need to interpret programs in two different ways (Hugo: In my understanding, you are advocating that a deep embedding of the proofs in Agda is more manageable than a shallow embedding? If so, this is standard nomenclature for PEPM, and deep embeddings are known for being more flexible.)}
