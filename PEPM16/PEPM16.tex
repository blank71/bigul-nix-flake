...
(A bigger example is given at the end of this section.)
The source is a list of book data of type |String × String × ℕ × ℕ × Bool| storing a book's title, author, publication year, price, and whether it is in stock, and we intend to update the price of those books published in 2015 which are still in stock.
...
for every matched pair of source and view books, we rearrange the view~(lines 3--4) to match with the shape of the source, and replace the title and price of the source with those from the view~(lines 5--6)\todo{pattern matching on source};
for every mismatched view book, we create a source book with default author information and publication year 2015 and mark it as in stock~(line~7), and its title and price will later be updated by the action for matched pairs;
finally, for every mismatched source book, we mark it as out of stock~(line~8).
...
Due to space limitation, in this paper we will only provide a sketch of the lens semantics of |BiGUL|.\todo{Revise: a relatively complete treatment for |caseS|, sketches for others}\
...
{-"\lno6"-}                  ((String ,  replace) , (⊤ ,  skip) , (⊤ ,  skip) , (ℕ ,  replace) , (⊤ ,  skip))))
...
For example, in our putback-based language \BiYacc~\cite{Zhu-BiYacc} for generating well-behaved pairs of parsers and ``reflective'' printers, a program specifies how a printer \emph{updates} a concrete syntax tree (the source) to become consistent with an abstract syntax tree (the view). % while preserving certain structures of the source (e.g., parentheses).
...
The example in \autoref{sec:global-corporation} shows a simpler scenario in which we also need this increased flexibility.
...
|GetPut| is then proved by setting |bs'| to |[]|, in which case the premise about |check-diversion| becomes vacuously true.
...
For mismatched views, we should create corresponding sources in the source list (so a subsequent |get| can produce these views).
...
Finally, for mismatched sources, we can choose to simply delete them or transform them such that they do not satisfy the |source-condition|.
This is specified by a function |conceal : S → Par (Maybe S)|: a mismatched source is deleted if applying |conceal| to it produces |nothing|, or transformed if |conceal| produces a new source wrapped in |just|.
...
\subsection{A complete example: transatlantic corporation}
\label{sec:global-corporation}
