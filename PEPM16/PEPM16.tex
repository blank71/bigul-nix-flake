...
\item Second, the well-behavedness of |BiGUL| has been completely formally verified using the dependently typed programming language \Agda~\cite{Norell-thesis,Nore08} (version 2.4.2.3 with standard library version~0.9), so now we can fully confidently guarantee that all the inserted dynamic checks can indeed help to establish well-behavedness.
...
\begin{code}
bind-computes :
  {A B : Set} (mx : Maybe A) {f : A → Maybe B} {y : B} →
  mx >>= f ≡ just y →
...
As for establishing the consequent, a probably easiest way is to rewrite |Lens.get l a'| to |just b'| with an application of |Lens.PutGet l| so the consequent simplifies to |Lens.get r b' ≡ just c|, which can then be discharged by an application of |Lens.PutGet r|.
The whole proof of |pg| is
\begin{code}
...
\end{code}
which is apparently more heavyweight than~\autoref{lambda-expression}, even for this small example about lens composition.
For more complicated well-behavedness proofs, the above naive approach quickly becomes unmanageable, whereas our approach scales nicely.
...
Having explained the infrastructure of our \Agda\ formalisation, we can now move on to the |BiGUL| language itself.
...
{-"\lno3"-}         (rearr  (prod var var)  (prod  var (prod    (const tt) (prod   (const tt) (prod  var          (const tt)))))
...
(A more complex example is given in \autoref{sec:transatlantic-corporation}.)
...
we focus on those books in the source list which is published in 2015 and still in stock~(line~1) and align them with the view list by title~(line~2);
...
for every unmatched view book, we create a source book with default author information and publication year 2015 and mark it as in stock~(line~7), and its title and price will later be updated by the action for matched pairs;
finally, for every unmatched source book, we mark it as out of stock~(line~8).
...
For each operation we will start from a natural semantics (for |put|), then add necessary checks so a corresponding |get| can exist, arriving at the lens semantics for the operation (including well-behavedness proofs), and finally deeply embed the lens as a constructor of |BiGUL|.
...
Its lens semantics can in fact be derived from a \emph{partial isomorphism}:
...
\end{code}
of which |Lens| is a generalisation, as we can easily convert an |Iso A B| to a |Lens A B|:
\begin{code}
  ...
  where
    ...
    put-with-adaptation ((p , b) ∷ bs)  bs' s v cont =
      p s >>= λ matched → if matched  then'  branch  (λ l →  Lens.put l s v >>= λ s' → p s' >>= λ matched' →
                                                     ...
                                                     (λ u → u s >>= cont) b
                                      else   put-with-adaptation bs ((p , b) ∷ bs') s v f
...
PutGet-with-adaptation :
  (bs bs' : List CaseSBranch) {v : V} {cont : S → Par S} →
  ({s s' : S} →  (cont s ↦ s') →
                 (get (revcat bs' bs) s' ↦ v)) →
  {s s' : S} →  (put-with-adaptation bs bs' s v cont ↦ s') →
                (get (revcat bs' bs) s' ↦ v)
\end{code}
...
\end{code}
which is the usual way to implement linear-time list reversal, and coincides with how we move branches from |bs| to |bs'|.
...
|PutGet| is proved by setting |bs'| to~|[]| and applying |PutGet-with-adaptation| to itself in a way similar to how we define |put| in terms of |put-with-adaptation|.
For |GetPut|, the generalisation we need is relatively simpler:
\begin{code}
GetPut-with-adaptation :
  (bs bs' : List CaseSBranch)
...
Thus, in the definition of |CaseVBranch| below\todo{morally just |Lens S V|?}, we use a partial isomorphism on the view type --- e.g., an instance of |pat-iso| for doing pattern matching\todo{forward reference}\ --- which converts it to a possibly less informative type, instead of just a decidable predicate as in |CaseSBranch|:
\begin{code}
...
\end{code}
The types that we can encode within~|U| are those expressible in terms of any types with a decidable equality, |⊤|, |_⊎_|\ucomma |_×_|\ucomma and |List|; applying |⟦_⟧| to an inhabitant of the universe decodes it to the type it represents.
...
\begin{code}
^^^update-lens : (HIDE({D : U} ->))  (pat : Pattern D) (ls : Lenses pat) ->
...
The kind of invertible computation we wish to express is a decomposition of an old view followed by an assembling of a new view using all components of the old view (e.g.,
\begin{code}
\ (title , price) -> (title , tt , tt , price)
\end{code}
at lines 3--4 in \autoref{fig:bookstore}).
...
view-rearrangement-iso : (HIDE({D E : U} ->))
  (pat : Pattern D) (pat' : Pattern E) (paths : Paths pat pat') →
  Invertible pat pat' paths → Iso ⟦ E ⟧ ⟦ D ⟧
\end{code}
...
\end{code}
where |^^^Name : U| and |^^^Salary : U| are datatypes representing name and salary encoded as elements of~|U| (by~|k|).
...
Most interestingly, when an employee is relocated to the other country, their salary should also be converted to the other currency.
...
Entries from the two lists are matched by employee name.
Pairs of matched |Employee| and |Office| entries are synchronised by an inner |BiGUL| statement |relocate-employee| defined later.
An unmatched |Office| entry means that a new employee is inserted, so we create a default |Employee| entry in the source list, which is then updated by |relocate-employee| using the unmatched |Office| entry.
(We put the view location in the default |Employee| entry merely for efficiency, as |relocate-employee| would not have to deal with mismatching source and view locations.)
An unmatched |Employee| entry means that the employee is removed from the view, so we conceal the entry by deleting it from the source.

Matched pairs of |Employee| and |Office| are synchronised by
\begin{code}
^^^relocate-employee : BiGUL Employee Office
relocate-employee =
  rearr  (prod var var)  (prod  var (prod    (const tt)  var))
                         (      inj₁ refl ,  tt ,        inj₂ refl)
           -- |\ { (name , loc) -> (name , tt , loc) }|
         (update  (prod   var              var)
                  ((_ ,   replace) , (_ ,  adjust-salary-and-office)))
\end{code}
First we rearrange the view to the same shape as the source; specifically, we insert |tt| between the name and location, to be paired with the salary in the source later.
Next we decompose the source by pattern matching to replace the name in the source with the name in the view --- this looks like a redundant step but is required because we enforce syntactically that all view information, in particular the name, must be put into the source.
...
If it is the US, we should adapt the source by changing the salary from dollars to pounds (by a function |$⇒£ : Salary -> Salary|) and change the location to an empty one in the UK, which will be replaced by the view location in the second round of the source case analysis following the adaptation.
The conditions |^^^inUK| and |^^^inUS| have type
\begin{code}
...
We should now check, from the inside out, whether all constraints for passing dynamic checking are met.
...
For the |caseV| in |adjust-salary-and-office|, the ranges of |relocate-to-UK| and |relocate-to-US| are disjoint since they respectively produce entries with UK and US locations.
For the |align| in |relocate-employees|, since the source condition is always true, whatever |relocate-employee| produces does satisfy the source condition, and we must conceal unmatched sources by deleting them, which is indeed what we specify; finally, we must guarantee that the source produced by |relocate-employee| and the view used should again satisfy the matching condition, even when the view is an unmatched one and the original source is a default one created from the view, and this is indeed guaranteed because |relocate-employee| puts the name in the view into the source.
